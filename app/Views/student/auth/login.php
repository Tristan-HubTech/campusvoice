...
<div class="cv-rp">
    ...
    <div class="cv-card">
        ...
        <!-- Segmented control -->
        <div class="cv-tabs" id="auth-tab-switch" role="tablist">
            <button type="button" class="cv-tab" data-auth-tab="login"    role="tab" aria-selected="false">Sign In</button>
            <button type="button" class="cv-tab" data-auth-tab="register" role="tab" aria-selected="false">Register</button>
            <div class="cv-tab-indicator" id="cv-tab-indicator" aria-hidden="true"></div>
        </div>
        ...
        <!-- Panes host -->
        <div class="auth-panes-host" id="auth-panes-host">

        <!-- ── SIGN IN PANE ── -->
        <section class="auth-pane <?= $authMode === 'register' ? 'auth-pane--inactive' : '' ?>"
                 data-auth-pane="login" role="tabpanel" id="pane-login">
            <div class="cv-head">
                <div class="cv-eyebrow">Welcome Back</div>
                <h2>Sign in to your account</h2>
                <p>Access your campus portal and stay connected.</p>
            </div>

            <form method="post" action="<?= site_url('users/login') ?>" class="cv-form"
                  novalidate autocomplete="off" id="login-form">
                ...
                <div>
                    ...
                    <small class="cv-err" id="login-email-err" role="alert"></small>
                </div>
                ...
                <div>
                    ...
                    <small class="cv-err" id="login-pass-err" role="alert"></small>
                </div>
                ...
                <div>
                    <button type="submit" class="cv-btn" id="login-submit">
                        Sign In &nbsp;→
                    </button>
                    <div class="cv-foot-row" style="margin-top:10px;">
                        <a href="<?= site_url('users/forgot-password') ?>" class="cv-link">Forgot password?</a>
                    </div>
                </div>
            </form>
        </section>
        ...
        <!-- ── REGISTER PANE ── -->
        <section class="auth-pane <?= $authMode !== 'register' ? 'auth-pane--inactive' : '' ?>"
                 data-auth-pane="register" role="tabpanel" id="pane-register">
            <div class="cv-head">
                <div class="cv-eyebrow">Join CampusVoice</div>
                <h2>Create your account</h2>
                <p>Fill in your details and verify your email to get started.</p>
            </div>

            <form method="post" action="<?= site_url('users/login') ?>" class="cv-form"
                  novalidate autocomplete="off" id="register-form">
                <input type="hidden" name="auth_mode" value="register">

                <div class="cv-row2">
                    <div>
                        <label for="reg-first">First Name</label>
                        <input id="reg-first" name="first_name" type="text" required
                               maxlength="100" placeholder="First name"
                               aria-describedby="reg-first-err">
                        <small class="cv-err" id="reg-first-err" role="alert"></small>
                    </div>
                    <div>
                        <label for="reg-last">Last Name</label>
                        <input id="reg-last" name="last_name" type="text" required
                               maxlength="100" placeholder="Last name"
                               aria-describedby="reg-last-err">
                        <small class="cv-err" id="reg-last-err" role="alert"></small>
                    </div>
                </div>
                ...
                <div>
                    ...
                    <input id="reg-email" name="email" type="email" required
                           maxlength="150" autocomplete="off" placeholder="you@university.edu"
                           aria-describedby="reg-email-err">
                    <small class="cv-err" id="reg-email-err" role="alert"></small>
                </div>

                <div>
                    <label for="reg-password">
                        Password <small style="font-weight:400;opacity:.55;">(min 8 chars)</small>
                    </label>
                    <input id="reg-password" name="password" type="password" required
                           minlength="8" maxlength="255" autocomplete="new-password"
                           placeholder="Create a password" aria-describedby="reg-pass-err">
                    <small class="cv-err" id="reg-pass-err" role="alert"></small>
                </div>
                ...
                <div>
                    ...
                    <small class="cv-err" id="reg-confirm-err" role="alert"></small>
                </div>

                <!-- Send OTP — enabled when all fields pass -->
                <div>
                    <button type="button" class="cv-btn-otp" id="send-register-otp-btn" disabled>
                    ...
                    <small id="reg-email-hint"      class="cv-otp-txt" aria-live="polite" style="margin-top:6px;"></small>
                    <small id="register-otp-status" class="cv-otp-txt" aria-live="polite" style="margin-top:2px;"></small>
                </div>

                <!-- OTP input — revealed after successful send -->
                <div id="reg-otp-section" class="cv-otp-reveal" style="display:none">
                    <label for="reg-otp">Verification Code</label>
                    <input id="reg-otp" name="otp" type="text" inputmode="numeric"
                           pattern="[0-9]*" required maxlength="6"
                           placeholder="Enter 6-digit code" disabled>
                </div>

                <button type="submit" class="cv-btn" id="register-submit">
                    Create Account &nbsp;→
                </button>
            </form>
        </section>
...
(function () {
    ...
    var panesHost  = document.getElementById('auth-panes-host');
    ...
    function isEmail(v) { return /^[^\s@]+@[^\s@]+\.[a-zA-Z]{2,}$/.test(v); }

    function showOtpStatus(msg, isErr) {
        if (!otpStatus) return;
    ...
    function setMode(mode, updateUrl, animate) {
        ...
        panes.forEach(function (p) {
            var active = p.getAttribute('data-auth-pane') === mode;
            p.classList.toggle('auth-pane--inactive', !active);
            if (active) incoming = p;
        });
        tabButtons.forEach(function (btn) {
            var active = btn.getAttribute('data-auth-tab') === mode;
            btn.classList.toggle('cv-tab--active', active);
        ...
        if (updateUrl) {
            // Dismiss global flash alerts when user explicitly switches tabs
            document.querySelectorAll('.cv-alert').forEach(function(el) { el.style.display = 'none'; });
    ...
    (function () {
        ...
        var eIn   = $id('login-email');
        var pIn   = $id('login-password');
        var eErr  = $id('login-email-err');
        var pErr  = $id('login-pass-err');
        var btn   = $id('login-submit');
        if (!form) return;

        function chkEmail() {
            var v = eIn ? eIn.value.trim() : '';
            if (!v)           { setErr(eIn, eErr, 'Email address is required.'); return false; }
            if (!isEmail(v))  { setErr(eIn, eErr, 'Enter a valid email address.'); return false; }
            clearErr(eIn, eErr); return true;
        }
        function chkPass() {
            var v = pIn ? pIn.value : '';
            if (!v) { setErr(pIn, pErr, 'Password is required.'); return false; }
            clearErr(pIn, pErr); return true;
        }

        if (eIn) {
            eIn.addEventListener('blur', chkEmail);
            eIn.addEventListener('input', function () { if (eIn.classList.contains('is-invalid')) chkEmail(); });
        }
        if (pIn) {
            pIn.addEventListener('blur', chkPass);
            pIn.addEventListener('input', function () { if (pIn.classList.contains('is-invalid')) chkPass(); });
        }
        form.addEventListener('submit', function (e) {
            if (!chkEmail() | !chkPass()) { e.preventDefault(); return; }
    ...
    (function () {
        var fields = {
            first:   { i: $id('reg-first'),    e: $id('reg-first-err')   },
            last:    { i: $id('reg-last'),      e: $id('reg-last-err')    },
            email:   { i: $id('reg-email'),     e: $id('reg-email-err')   },
            pass:    { i: $id('reg-password'),  e: $id('reg-pass-err')    },
            confirm: { i: $id('reg-confirm'),   e: $id('reg-confirm-err') }
        };

        function vld(key) {
            var f = fields[key]; if (!f || !f.i) return true;
            var v = f.i.value;
            if (key === 'first' || key === 'last') {
                if (!v.trim()) { setErr(f.i, f.e, 'This field is required.'); return false; }
                clearErr(f.i, f.e); return true;
            }
            if (key === 'email') {
                if (!v.trim())   { setErr(f.i, f.e, 'Email is required.'); return false; }
                if (!isEmail(v)) { setErr(f.i, f.e, 'Enter a valid email address.'); return false; }
                clearErr(f.i, f.e); return true;
            }
            if (key === 'pass') {
                if (!v)           { setErr(f.i, f.e, 'Password is required.'); return false; }
                if (v.length < 8) { setErr(f.i, f.e, 'Password must be at least 8 characters.'); return false; }
                clearErr(f.i, f.e);
                if (fields.confirm.i && fields.confirm.i.value) vld('confirm');
                return true;
            }
            if (key === 'confirm') {
                var pw = fields.pass.i ? fields.pass.i.value : '';
                if (!v)      { setErr(f.i, f.e, 'Please confirm your password.'); return false; }
                if (v !== pw){ setErr(f.i, f.e, 'Passwords do not match.'); return false; }
                clearErr(f.i, f.e); return true;
            }
            return true;
        }
        ...
        Object.keys(fields).forEach(function (key) {
            var f = fields[key]; if (!f.i) return;
            f.i.addEventListener('blur',  function () { vld(key); });
            f.i.addEventListener('input', function () {
                if (f.i.classList.contains('is-invalid')) vld(key);
    ...
    if (sendOtpBtn) {
        ...
        function gather() {
            return {
                fn:  ($id('reg-first')    || {}).value || '',
                ln:  ($id('reg-last')     || {}).value || '',
                em:  ($id('reg-email')    || {}).value || '',
                pw:  ($id('reg-password') || {}).value || '',
                cp:  ($id('reg-confirm')  || {}).value || ''
            };
        }

        function isComplete(v) {
            return v.fn && v.ln && v.em && v.pw && v.cp
                && isEmail(v.em) && v.pw.length >= 8 && v.pw === v.cp;
        }

        function updateOtpButtonState() {
            var v = gather();
            var ok = isComplete(v);
            sendOtpBtn.classList.toggle('ready', ok);
            sendOtpBtn.disabled = !ok;
            var hint = $id('reg-email-hint');
            if (hint) {
                var msg = '';
                if (!ok && isEmail(v.em)) {
                    if (!v.fn || !v.ln)       msg = '↑ Fill in your name to continue.';
                    else if (!v.pw)            msg = '🔒 Create a password to continue.';
                    else if (v.pw.length < 8)  msg = '🔒 Password needs at least 8 characters.';
                    else if (v.pw !== v.cp)    msg = '⚠ Passwords don\'t match yet.';
                }
                hint.textContent = msg;
                hint.style.color = msg ? 'rgba(253,230,138,.82)' : '';
            }
        }

        function requestOtp() {
            var v = gather();
            if (!isComplete(v)) { showOtpStatus('Please fill in all fields correctly.', true); return; }
            var body = new URLSearchParams();
            body.set('first_name', v.fn); body.set('last_name', v.ln);
            body.set('email', v.em); body.set('password', v.pw); body.set('password_confirm', v.cp);
            sendOtpBtn.disabled = true;
            showOtpStatus('Sending verification code…', false);
            ...
            .then(function (data) {
                var ok = !!(data && data.ok);
                showOtpStatus((data && data.message) || (ok ? 'Code sent — check your inbox.' : 'Unable to send code.'), !ok);
                if (ok && otpInput) {
                    otpInput.disabled = false;
                    var sec = $id('reg-otp-section');
                    if (sec) {
                        sec.style.display = 'block';
                        var ap = document.querySelector('[data-auth-pane="register"]:not(.auth-pane--inactive)');
                        if (ap && panesHost) panesHost.style.height = ap.scrollHeight + 'px';
                    }
                    otpInput.focus();
                }
            })
            .catch(function () { showOtpStatus('Failed to send code. Try again.', true); })
    ...
    (function () {
        var form = $id('register-form'), btn = $id('register-submit');
        if (form && btn) form.addEventListener('submit', function () { btn.classList.add('is-loading'); });
    }());
    ...
    (function () {
        var errBanner = document.querySelector('.cv-alert--error');
        if (otpInput && otpInput.value && errBanner) {
            otpInput.value = ''; otpInput.disabled = false;
            otpInput.style.borderColor = '#e53e3e';
            setTimeout(function () { otpInput.style.borderColor = ''; }, 900);
            otpInput.focus();
        }
    }());
}());
